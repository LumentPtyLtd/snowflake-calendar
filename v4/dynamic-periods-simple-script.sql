/*==============================================================================
  SIMPLE DYNAMIC PERIODS GENERATOR
  
  This script creates a dynamic periods view that adds time-relative flags and
  calculations to any calendar. These flags automatically update based on the 
  current system time and are essential for time-based filtering in reports and
  dashboards. 
  
  The simple dynamic periods view includes:
  - Current period flags (is_today, is_this_month, etc.)
  - Relative period flags (is_last_month, is_month_minus_2, etc.)
  - Rolling period flags (is_last_30_days, is_last_90_days, etc.)
  - Relative distance calculations (days_from_today, etc.)
  - Period descriptions for user displays
  
  USAGE:
  ------
  CALL GENERATE_SIMPLE_DYNAMIC_PERIODS(
      'UNIFIED_CALENDAR_TABLE_DAY',  -- Base calendar table/view name
      'DAY',                         -- Time grain
  ...
  PARAMETERS:
  -----------
  BASE_CALENDAR_NAME: The name of the underlying calendar table/view
  ...
  NOTES:
  ------
  - This view uses the current system date/time for all calculations
  - Changes to underlying calendar structure require regeneration of this view
  - For multi-timezone support, use GENERATE_DYNAMIC_PERIODS instead
...
$$
    ...
    try {
        ...
        const periodFlags = `
            ...
            -- Dates in the future have positive values, past dates negative
        ...
        // Period descriptions for user-friendly labels
        ...
        const viewComment = `Simple dynamic time periods view based on ${grainUpper} grain with ${RELATIVE_PERIODS_COUNT} relative periods. Uses the system current date for all calculations. The flags in this view automatically update based on the current date, making it ideal for date-relative filtering in reports and dashboards.`;
        ...
        return `Simple Dynamic Periods view '${finalViewName}' generated successfully based on '${BASE_CALENDAR_NAME}'. This view provides automatically updating time-relative flags (is_today, is_this_month, etc.) and period indicators for reporting and analytics. Access the view with SELECT * FROM ${finalViewName}.`;
