/*==============================================================================
  LOAD_AU_HOLIDAYS - AUSTRALIAN PUBLIC HOLIDAYS LOADER

  This script configures Snowflake external access and defines a Python stored
  procedure to fetch Australian public holidays from data.gov.au, transform the
  data, and load it into the `PUBLIC_HOLIDAYS_AU` table.
  ...
  FEATURES:
  ---------
  - Creates network rule and external access integration
  - Fetches holiday data via HTTPS API call
  - Loads data into `PUBLIC_HOLIDAYS_AU` table with metadata
  - Can be rerun to refresh data

  NOTES:
  ------
  - Requires Snowflake external network access enabled
  - Uses Python 3.11, Snowpark, pandas, requests
  - Adjust database/schema as needed
  - Table is append-only by default; modify as required
...
STAGE_NAME = "PUBLIC_HOLIDAYS_AU_STAGE" # Name of the Snowflake stage used for temporary data storage.
TABLE_NAME = "PUBLIC_HOLIDAYS_AU" # Name of the Snowflake table where the holiday data is loaded.
