...
$$
    ...
    try {
        ...
        addColumn('RETAIL_YEAR', retailYearExpr, 'Retail Year identifier');
        ...
        addColumn('RETAIL_YEAR_START_DATE', startOfRetailYearExpr, 'Start date of the Retail Year');

        // *** Determine End Date based on potential 53 weeks ***
        // Find the start of the *next* retail year first
        const startOfNextRetailYearExpr = `DATE(DATE_TRUNC('WEEK', DATE_FROM_PARTS(${retailYearExpr} + 1, ${endMonthInt + 1}, 1)))`;
        // The end date is simply the day before the next year starts
        const endOfRetailYearExpr = `DATEADD(day, -1, ${startOfNextRetailYearExpr})`;
        addColumn('RETAIL_YEAR_END_DATE', endOfRetailYearExpr, 'End date of the Retail Year (day before next retail year starts).');
        ...
        addColumn('RETAIL_WEEK_OF_YEAR', retailWeekOfYearExpr, 'Retail Week number (1-53).');
        ...
        const retailMonthCaseExpr = `
            ...
            END`;
        addColumn('RETAIL_MONTH_OF_YEAR', retailMonthCaseExpr, `Retail month number (1-12) based on ${patternUpper}, week 53 assigned to M12.`);
        ...
        const retailQuarterCaseExpr = `
            ...
            END`;
        addColumn('RETAIL_QUARTER', retailQuarterCaseExpr, `Retail quarter number (1-4) based on ${patternUpper}, week 53 assigned to Q4.`);
        ...
        return "Retail Calendar view generated successfully: " + retailViewName + " based on " + baseViewName;

    } catch (err) {
        return `ERROR creating view ${retailViewName}: ${err.code} - ${err.message}\nAttempted SQL:\n${createViewSql}`;
    }
$$
;